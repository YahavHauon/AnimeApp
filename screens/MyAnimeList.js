...
const MyAnimeList = ({ navigation }) => {
  const { myAnimeListState, deleteAnime, deleteAllAnime, myAnimeListProvider } =
  ...
  const listHiddenRenberItem = (data) => {
    return (
      <Pressable onPress={() => deleteAnime(data.item.mal_id)}>
  ...
  const deletAll = () => {
    Alert.alert(
      ...
      [{ text: "Im gay" }, { text: "Sure", onPress: () => deleteAllAnime() }]
    );
  };
  return (
    <View
      style={[
        styles.container,
        myAnimeListState && myAnimeListState?.length > 0
          ? null
          : styles.justifyCenter,
      ]}
    >
      {myAnimeListState && myAnimeListState?.length > 0 ? (
        <Button style={styles.button} onPress={deletAll}>
          Delete all
        </Button>
      ) : null}
